...
class gunlun:
        ...
        def __init__(self):
                ...
                # self.backpng = tk.Canvas(self.window, height=800, width=1024,bg='#986DB2')#创建画布
                ...
                self.photo = tk.PhotoImage(file="BACK3GIF.gif")
                ...
                self.rolling_name_show = tk.Entry(self.window, fg='Crimson',font=('宋体', 28),textvariable=self.rolling_name, width=20)
                # self.rolling_name_show.pack(side='top')
                self.rolling_name_show.place(x=290, y=150)
                ...
                self.btn_start = tk.Button(self.window,bg='FireBrick',fg='Snow', font=('Kozuka Gothic Pr6N B', 28),textvariable=self.button_start_str, command=self.rolling_start)
